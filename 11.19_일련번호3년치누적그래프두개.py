...
cur = con.cursor()
print("=== 월 ====== 단체 ===== 그룹 =====레저====")
cur.execute("select id, sum(d_individual) OVER(order by id), sum(d_group) OVER(order by id), sum(d_leisure) OVER(order by id) as dum_of_d_group \
                from jejutourist2\
                order by id;") 
rows = cur.fetchall()
print(rows)
...
cur.execute("select id, sum(f_china) OVER(order by id),sum(f_asia_etc) OVER(order by id),sum(f_usa) OVER(order by id) \
                from jejutourist2\
                order by id;") 
rows = cur.fetchall()
print(rows)
